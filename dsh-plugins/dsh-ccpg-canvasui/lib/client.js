...
//   右 = 轻量聊天记录栏（自绘，/api/session.history 轮询）——官方 renderSlot 被封在
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ensureStyle() {
			...
			el.textContent = [
				...
				".wf1-split[data-chat-collapsed='1'] .wf1-split-chat{display:none;}",
					...
					".wf1-split[data-chat-collapsed='1'] .wf1-chat-toggle{top:50%;right:0;transform:translateY(-50%);width:42px;height:132px;padding:12px 9px;border-radius:10px 0 0 10px;border-right:0;background:var(--dsw-alias-state-business-primary,#8B5CF6);color:#fff;writing-mode:vertical-rl;text-orientation:upright;letter-spacing:2px;box-shadow:-3px 0 14px rgba(0,0,0,.4);}",
					".wf1-split[data-chat-collapsed='1'] .wf1-chat-toggle:hover{filter:brightness(1.12);}",
					".wf1-chat-head{flex:none;padding:10px 14px;border-bottom:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.08));display:flex;align-items:center;gap:8px;}",
				".wf1-chat-title{font-size:12px;font-weight:600;color:var(--dsw-alias-label-secondary);letter-spacing:.02em;}",
				".wf1-chat-list{flex:1;min-height:0;overflow-y:auto;padding:10px 12px;display:flex;flex-direction:column;gap:10px;}",
				".wf1-msg{max-width:100%;padding:8px 10px;border-radius:10px;font-size:13px;line-height:1.55;white-space:pre-wrap;word-break:break-word;}",
				".wf1-msg-user{align-self:flex-end;background:var(--dsw-alias-state-business-primary,#8B5CF6);color:#fff;border-bottom-right-radius:4px;}",
				".wf1-msg-ai{align-self:flex-start;background:var(--dsw-alias-bg-raised,#26221D);color:var(--dsw-alias-label-primary,#e8e4dc);border-bottom-left-radius:4px;}",
				".wf1-msg-tool{align-self:flex-start;background:transparent;border:1px dashed var(--dsw-alias-border-l2,rgba(255,255,255,.14));color:var(--dsw-alias-label-tertiary,#8b8578);font-size:12px;font-family:ui-monospace,monospace;padding:5px 9px;}",
				".wf1-msg-tool b{color:var(--dsw-alias-state-business-primary,#8B5CF6);font-weight:600;}",
				".wf1-chat-hint{flex:none;padding:6px 12px;border-top:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.08));font-size:11px;color:var(--dsw-alias-label-tertiary,#8b8578);text-align:center;}",
			].join("\n");
		...
		// ---- 聊天记录半：/api/session.history 轮询（信封同官方 client）----
		var rpcSeq = 0;
		function apiCall(method, payload) {
			...
			return fetch("/api/" + method, {
				...
				body: JSON.stringify({ type: "client-request", rpcId: "wf1c" + rpcSeq, method: method, payload: payload }),
		...
		function historyToItems(value) {
			...
			for (var i = 0; i < events.length; i += 1) {
				...
				if (ev.type === "user/message") {
					...
					var at = textOf(data.message && data.message.content);
					...
					try {
						...
						if (name === "canvas_graph_patch" && a && a.ops) argStr = a.ops.length + " 个操作";
						else argStr = JSON.stringify(a || {}).slice(0, 60);
		...
		function ChatPane(props) {
			...
			react.useEffect(function () {
				...
				var tick = function () {
					if (stop) return;
					apiCall("session.history", { sessionId: sessionId, maxMessages: 60 }).then(function (v) {
						if (stop) return;
						setItems(historyToItems(v));
						setErr(null);
					}).catch(function (e) {
			...
			return react.createElement("div", { style: { display: "flex", flexDirection: "column", height: "100%", minHeight: 0 } },
				react.createElement("div", { className: "wf1-chat-head" },
					react.createElement("span", { className: "wf1-chat-title" }, "对话记录"),
					err ? react.createElement("span", { style: { fontSize: 11, color: "#ef4444" } }, "同步失败") : null,
				),
				react.createElement("div", { className: "wf1-chat-list", ref: listRef, onScroll: onScroll },
					items.length === 0 ? react.createElement("div", { style: { color: "#8b8578", fontSize: 12, textAlign: "center", marginTop: 20 } }, "暂无消息") : null,
					items.map(function (it, idx) {
						if (it.kind === "tool") {
							return react.createElement("div", { className: "wf1-msg wf1-msg-tool", key: idx },
								"⚙ ", react.createElement("b", null, it.name), " ", it.text);
						}
						return react.createElement("div", {
							className: "wf1-msg " + (it.kind === "user" ? "wf1-msg-user" : "wf1-msg-ai"),
							key: idx,
						}, it.text);
					}),
				),
				react.createElement("div", { className: "wf1-chat-hint" }, "输入框在下方 · 切到「对话」tab 看完整交互"),
			);
		}
			...
			var toggleChat = function () {
				...
				return react.createElement("div", { className: "wf1-view-root" },
					react.createElement("div", {
						...
						collapsed ? null : react.createElement("div", { className: "wf1-split-chat", style: { width: chatPx + "px" } },
